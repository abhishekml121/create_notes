...
if(is_get_request()){
	...
	if($u_note !== false){
		$send_arr = array('type'=>'get_user_notes_title','result'=>'true','user_notes_html'=> $u_note);
		if($current_page == 1){
			$send_arr['top_nav'] = htmlMarkup::nav_bar();
		}
	}elseif($current_page == 1 && $u_note == false){
		$send_arr = array('type'=>'get_user_notes_title','result'=>'false','message'=> htmlMarkup::zero_note_message());
	}elseif($u_note === false){
		$send_arr = array('type'=>'get_user_notes_title','result'=>'false','message'=>'No more notes to show');
	}else{
		$send_arr = array('type'=>'get_user_notes_title','result'=>'false','error'=> 'Can not load notes');
	}
